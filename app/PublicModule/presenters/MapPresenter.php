...
final class MapPresenter extends BasePresenter
{
    ...
    {
        $this->redirect('Map:Category', 'doprava');
    }
    ...
    public function renderCategory($id, $page = 1)
    {
        ...
        $this->template->datasets_count = $this->db->table('dataset')
            ...
            ->where(':dataset_files.map IS NOT NULL OR dataset.map IS NOT NULL')
            ->count();
    ...
    public function renderShow($id)
    {
        ...
        foreach ($this->template->files as $f) {
            if (!empty($f->map))
                $mp = true;
        }

        if (!$mp && empty($this->template->d->map))
            return $this->redirect('Homepage:');
    }
